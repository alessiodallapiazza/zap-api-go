...
//
// This component is optional and therefore the API will only work if it is installed
func (g Graphql) OptionQueryGenEnabled() (map[string]interface{}, error) {
	return g.c.Request("graphql/view/optionQueryGenEnabled/", nil)
}
...
//
// This component is optional and therefore the API will only work if it is installed
func (g Graphql) SetOptionQueryGenEnabled(boolean bool) (map[string]interface{}, error) {
	m := map[string]string{
		"Boolean": strconv.FormatBool(boolean),
	}
	return g.c.Request("graphql/action/setOptionQueryGenEnabled/", m)
}
